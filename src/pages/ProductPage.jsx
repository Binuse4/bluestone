...
export default function ProductPage() {
  ...
  // --- RENDU THÈME NORDIC ---
  if (template === 'nordic') {
    const thumbnails = (product.product_images || []).filter(Boolean);
        ...
        <div className="nordic-product-layout">
          ...
          <div className="nordic-gallery">
            <img
              ...
              alt={product.name}
              className="nordic-gallery-main-img"
              style={{ filter: product.is_available === false ? 'grayscale(1)' : 'none' }}
            />
            {thumbnails.length > 1 && (
              <div className="nordic-thumb-row">
                {thumbnails.map((img, i) => (
                  <button
                    key={i}
                    onClick={() => product.colors?.[i] && setSelectedColor(product.colors[i])}
                    className={`nordic-thumb${currentMainImage === img ? ' active' : ''}`}
                  >
                    <img src={img} alt={product.colors?.[i] || ''} />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="nordic-info-panel">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' }}>
              ...
              <button onClick={handleLikeToggle} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '5px', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
              ...
              {product.compare_price && (
                <span style={{ textDecoration: 'line-through', color: '#A0AEC0', fontSize: '1rem' }}>
                  {product.compare_price.toLocaleString()} {product.currency || 'FCFA'}
                </span>
              )}
            </div>

            <p className="nordic-desc">{product.description}</p>

            {/* Tailles */}
            {product.sizes && product.sizes.length > 0 && product.sizes[0] !== 'Standard' && (
              <div className="nordic-option-row">
                <span className="nordic-option-label">Taille</span>
            ...
            {product.colors && product.colors.length > 0 && product.colors[0] !== 'Unique' && (
              <div className="nordic-option-row">
                <span className="nordic-option-label">Couleur</span>
            ...
            {/* Quantité */}
            <div className="nordic-option-row">
              <span className="nordic-option-label">Quantité</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="nordic-qty-arrow">&#9664;</button>
                <span style={{ width: '32px', textAlign: 'center', fontWeight: 600, color: '#1a202c' }}>{quantity}</span>
                <button onClick={() => setQuantity(q => q + 1)} className="nordic-qty-arrow">&#9654;</button>
              </div>
            </div>
