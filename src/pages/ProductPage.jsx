...
export default function ProductPage() {
  ...
  if (!product) {
    return (
      <div className="container not-found-layout">
        ...
        <p className="not-found-text">Le produit recherché n'existe pas ou n'est plus disponible.</p>
        <Link to={`/c/catalogue/${store.slug}/explore`} className="btn btn-primary">Retourner au catalogue</Link>
      </div>
    );
  }
  ...
  const getColorDot = (colorName) => {
    const colorsMap = {
      'blanc': '#ffffff', 'noir': '#1a1a1a', 'ocre': '#b45309', 'indigo': '#3f51b5',
      'marron': '#8c6239', 'camel': '#c19a6b', 'bronze': '#cd7f32', 'or': '#ffd700', 'argent': '#c0c0c0'
    };
    return colorsMap[colorName.toLowerCase()] || null;
  };
      ...
      <div className="container mr-product-page fade-in">
        ...
        <div className="mr-info-panel">
          <div className="mr-info-header">
            ...
            <p className="mr-product-price">{formattedPrice}</p>
          </div>
          <div className="mr-description">
            <p>{product.description} <span className="mr-read-more">lire plus</span></p>
          </div>
          {/* Colors section */}
            ...
            <div className="mr-colors-section">
              ...
              <div className="mr-colors-row">
                {product.colors.map(color => {
                  const dotColor = getColorDot(color);
                  return (
                    <button 
                      key={color} 
          ...
          <div className="refine-product-gallery">
            ...
            {images.length > 1 && (
              <div className="refine-image-scroll">
                {images.map((img, idx) => (<div key={idx} className="refine-scroll-item"><img src={img} alt={`${product.name} ${idx}`} /></div>))}
              </div>
            )}
          </div>
          <div className="refine-product-info">
            <div className="refine-info-top"><h1 className="refine-product-name">{product.name}</h1><p className="refine-product-price">{formattedPrice}</p></div>
            ...
            <div className="refine-options">
              {product.sizes && product.sizes.length > 0 && product.sizes[0] !== 'Standard' && (
                <div className="refine-option-item">
                  <span className="refine-option-label">Taille</span>
    ...
    return (
      <div className="container product-detail-layout modern-product-page fade-in">
        ...
        <div className="modern-gallery-section">
          <div className="modern-main-image-wrapper"><img src={images[0]} alt={product.name} className="modern-main-img" /><div className="shoe-depth-shadow-large"></div></div>
          {images.length > 1 && (
            <div className="modern-thumbnails">{images.map((img, idx) => (<div key={idx} className={`modern-thumb ${idx === 0 ? 'active' : ''}`}><img src={img} alt={`${product.name} ${idx}`} /></div>))}</div>
          )}
        </div>
        <div className="modern-info-panel">
          ...
          <div className="modern-price-row"><span className="modern-price-value">{formattedPrice}</span><div className="modern-rating-badge"><svg width="14" height="14" viewBox="0 0 24 24" fill="#FFD700" stroke="#FFD700"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg><span>4.8 (120 avis)</span></div></div>
      ...
      <div className="container minimal-product-page fade-in">
        ...
        <div className="minimal-product-grid">
          ...
          <div className="minimal-product-info">
            ...
            <div className="minimal-product-price">{formattedPrice}</div>
    ...
    <div className="container fade-in" style={{ paddingBottom: 80 }}>
      ...
      <div className="product-detail-layout">
        ...
        <div className="product-info-panel">
          <div>{category && <span className="product-info-category">{category.name}</span>}<h1 className="product-info-name">{product.name}</h1><div className="product-info-price">{formattedPrice}</div></div>
