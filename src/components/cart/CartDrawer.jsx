...
export default function CartDrawer({ isOpen, onClose, store }) {
  ...
  if (template === 'nordic') {
    return (
      <div className={`cart-drawer-overlay ${isOpen ? 'open' : ''}`} onClick={onClose}>
        <div className="cart-drawer nordic-cart" onClick={(e) => e.stopPropagation()} ref={cartRef}>
          <div className="cart-drawer-header" style={{ borderBottom: '1px solid #E2E8F0', padding: '30px 24px' }}>
            <h3 className="nordic-cart-title" style={{ fontSize: '1.1rem', margin: 0 }}>Panier ({cartCount})</h3>
            <button className="cart-drawer-close" onClick={onClose} style={{ color: '#4A5568' }}>
          ...
          <div className="cart-drawer-body">
            {cart.length === 0 ? (
              <div className="cart-empty-state">
                <p style={{ color: '#718096', textTransform: 'uppercase', letterSpacing: '0.05em', fontSize: '0.85rem' }}>Votre panier est vide.</p>
                <button onClick={onClose} style={{ marginTop: '20px', background: 'none', border: '1px solid #1a202c', color: '#1a202c', padding: '10px 25px', textTransform: 'uppercase', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>Continuer les achats</button>
              </div>
            ) : (
              cart.map((item) => (
                <div key={item.cartItemId} className="cart-item fade-in" style={{ borderBottom: '1px solid #E2E8F0', paddingBottom: '20px', marginBottom: '20px' }}>
                  <div style={{ width: '90px', height: '120px', flexShrink: 0, backgroundColor: '#F7FAFC' }}>
                    <img src={item.image_url} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                  <div className="cart-item-details" style={{ justifyContent: 'flex-start', gap: '8px' }}>
                    <div className="cart-item-header">
                      <h4 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 500, color: '#2D3748' }}>{item.name}</h4>
                      <button onClick={() => removeFromCart(item.cartItemId)} style={{ background: 'none', border: 'none', color: '#A0AEC0', cursor: 'pointer', padding: 0 }}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2M10 11v6M14 11v6"/></svg></button>
                    </div>
                    <p style={{ margin: 0, fontSize: '0.75rem', color: '#718096', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      {item.selectedSize !== 'Standard' && `${item.selectedSize} `}
                      {item.selectedColor !== 'Unique' && `/ ${item.selectedColor}`}
                    </p>
                    <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: 600, color: '#4A5568' }}>{item.price.toLocaleString()} {item.currency || 'FCFA'}</p>
                    
                    <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #E2E8F0', width: 'fit-content', marginTop: 'auto' }}>
                      <button onClick={() => updateQuantity(item.cartItemId, item.quantity - 1)} style={{ width: '30px', height: '30px', background: 'none', border: 'none', cursor: 'pointer', color: '#4A5568' }}>-</button>
                      <span style={{ width: '30px', textAlign: 'center', fontSize: '0.85rem', color: '#1a202c' }}>{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.cartItemId, item.quantity + 1)} style={{ width: '30px', height: '30px', background: 'none', border: 'none', cursor: 'pointer', color: '#4A5568' }}>+</button>
                    </div>
                  </div>
                </div>
          ...
          {cart.length > 0 && (
            <div className="cart-drawer-footer" style={{ backgroundColor: '#fff', borderTop: '1px solid #E2E8F0', padding: '30px 24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
                <span style={{ fontSize: '0.9rem', color: '#4A5568', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total</span>
                <span style={{ fontSize: '1.2rem', fontWeight: 700, color: '#1a202c' }}>{finalTotal.toLocaleString()} {store?.currency || 'FCFA'}</span>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <button onClick={handleDownloadCart} style={{ background: 'none', border: '1px solid #E2E8F0', color: '#4A5568', padding: '15px', textTransform: 'uppercase', fontSize: '0.8rem', fontWeight: 600, letterSpacing: '0.05em', cursor: 'pointer', transition: 'border-color 0.2s' }} onMouseOver={(e) => e.target.style.borderColor = '#1a202c'} onMouseOut={(e) => e.target.style.borderColor = '#E2E8F0'}>
                  Télécharger mon panier
                </button>
                <button onClick={handleWhatsAppCheckout} className="whatsapp-nordic-btn" style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px' }}>
              ...
              <p className="cart-drawer-note" style={{ textAlign: 'center', marginTop: 15, fontSize: '0.7rem', color: '#A0AEC0', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Téléchargez votre panier en PNG avant de commander.</p>
            </div>
          )}
        </div>
      </div>
